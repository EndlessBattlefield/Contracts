...
contract EBCN is Context, ERC165, IERC1155, IERC1155MetadataURI,Pausable {
    ...
    function _balanceOfAddressBatch(
        address  account,
        uint256[] memory ids
    )
        internal
        ...
        returns (uint256[] memory)
    {
        ...
        require(account != address(0), "ERC1155: batch balance query for the zero address");
...
contract EBBasicNFT is EBCN,Roles{
    ...
    function mint(address account, uint256 id, uint256 amount,string memory _uri, bytes memory data) public onlyMinter{
        require(id != 0, "ERC1155: id cannot be 0");
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts,string[] memory _uris, bytes memory data) public onlyMinter{
        
        require(ids.length == amounts.length && ids.length == _uris.length, "ERC1155: ids and amounts and uris length mismatch");
        for (uint i = 0; i < ids.length; i++) {
    ...
    function mintAddressBatch(address[] memory accounts, uint256[] memory ids, uint256[] memory amounts,string[] memory _uris, bytes memory data) public onlyMinter{
        
        require(ids.length == amounts.length && ids.length == _uris.length && ids.length == accounts.length, "ERC1155: ids and amounts and uris and accounts length mismatch");
        for (uint i = 0; i < ids.length; i++) {
            if(bytes(_uris[i]).length > 0)
                uris[ids[i]] = _uris[i];
        }
        
        _mintAddressBatch(accounts,ids,amounts,data);
        
    }
 
    
    function burn(address account, uint256 id, uint256 amount) public onlyBurn{
        _burn(account,id,amount);
    }
 
    
    function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) public onlyBurn{
        _burnBatch(account,ids,amounts);
    }
    
    
    function balanceOfAddressBatch(address account,uint256[] memory ids) public view returns (uint256[] memory) 
    {
        
        return _balanceOfAddressBatch(account,ids);
    }
